...
struct SecureStorage {
    ...
    #[serde(alias = "selected_pluely_model")]
    selected_omni_model: Option<String>,
}
...
pub async fn secure_storage_save(app: AppHandle, items: Vec<StorageItem>) -> Result<(), String> {
    ...
    for item in items {
        match item.key.as_str() {
            "omni_license_key" | "pluely_license_key" => storage.license_key = Some(item.value),
            "omni_instance_id" | "pluely_instance_id" => storage.instance_id = Some(item.value),
            "selected_omni_model" | "selected_pluely_model" => storage.selected_omni_model = Some(item.value),
...
pub async fn secure_storage_get(app: AppHandle) -> Result<StorageResult, String> {
    ...
    Ok(StorageResult {
        ...
        selected_omni_model: storage.selected_omni_model,
    })
}
...
pub async fn secure_storage_remove(app: AppHandle, keys: Vec<String>) -> Result<(), String> {
    ...
    for key in keys {
        match key.as_str() {
            "omni_license_key" | "pluely_license_key" => storage.license_key = None,
            "omni_instance_id" | "pluely_instance_id" => storage.instance_id = None,
            "selected_omni_model" | "selected_pluely_model" => storage.selected_omni_model = None,
